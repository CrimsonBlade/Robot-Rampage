    ...
    static class GoalManager
    {
        ...
        public static void DetectShutdowns()
        {
            foreach (ComputerTerminal terminal in computerTerminals)
            {
                if (terminal.Active)
                {
                    if (terminal.IsCircleColliding(Player.BaseSprite.WorldCenter, Player.BaseSprite.CollisionRadius))
                    {
                        terminal.Deactivate();
                        activeCount--;
                    }
        ...
        public static void AddComputerTerminal()
        {
            ...
            if (path != null)
            {
                ...
                float timerOffest = (float)rand.Next(1, 100);
                terminal.LastSpawnCounter = timerOffest / 100f;
